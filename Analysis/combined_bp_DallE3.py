...
# Filter rows where "Image_Tool" is "DallE3"
cgpt_df = cgpt_df.loc[cgpt_df["Image_Tool"] == "DallE3"]
...
# Filter rows where "Image_Tool" is "DallE3"
gemini_df = gemini_df.loc[gemini_df["Image_Tool"] == "DallE3"]
...
sns.boxplot(
    ...
    data=combined_df,
    palette={
        "Correct (by GPT-4o)": "#f95f22",
        "Incorrect (by GPT-4o)": "#fcbca3",
        "Correct (by Gemini)": "#f95f22",
        "Incorrect (by Gemini)": "#fcbca3"
    },
...
for modality, count in sample_sizes.items():
    plt.text(
        ...
        y=67,  # Place the text slightly below the y-axis maximum
        s=f"n = {count}",
        ha="center",
        fontsize=10,
        color="black"
...
plt.title("DALL-E 3 Image Generation Time by Classification Result")
