...
gemini_csv_files = [
    "Stats_summary_bummer_combined_gemini_classify_text.csv",
    "Stats_summary_disaster_combined_gemini_classify_text.csv",
    "Stats_summary_glitch_combined_gemini_classify_text.csv"
]
...
cgpt_df["Time"] = pd.to_numeric(cgpt_df["Time_Script"], errors="coerce")  # Ensure numeric values
gemini_df["Time"] = pd.to_numeric(gemini_df["Time_Script"], errors="coerce")  # Ensure numeric values
...
sns.boxplot(
    ...
    data=combined_df,
    palette={
        "Correct (by GPT-4o)": "#b978c1",
        "Incorrect (by GPT-4o)": "#ddc0e0",
        "Correct (by Gemini)": "#b978c1",
        "Incorrect (by Gemini)": "#ddc0e0"
    },
...
plt.ylim(0, 14)  # Set the y-axis maximum to 14
...
for modality, count in sample_sizes.items():
    plt.text(
        ...
        y=13,  # Place the text slightly below the y-axis maximum
        s=f"n = {count}",
        ha="center",
        fontsize=10,
        color="black"
...
plt.title("Script Generation Time by Classification Result")
